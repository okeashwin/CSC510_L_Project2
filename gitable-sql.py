...
def dumpIssue(u,issues, token):
  ...
  for event in w:
    identifier = event['id']
    issue_id = event['number']
    issue_name = event['title']
    created_at = secs(event['created_at'])
    ...
    #              milestone = milestone)
    issue_obj = issues.get(issue_id)
    if not issue_obj: issue_obj = [issue_name, [],created_at,closed_at,milestone_id,assignee,label_str,labels_count]
    # all_events = issue_obj[1]
    # all_events.append(eventObj)
    issues[issue_id] = issue_obj
  return True
...
def dumpIs(u,issues,token):
  try:
    return dumpIssue(u, issues, token)
  except Exception as e:
    print(u)
    print(e)
    print("Contact TA")
...
def launchDump():
  ...
  conn.execute('''CREATE TABLE IF NOT EXISTS issue(id INTEGER, name VARCHAR(128),
        createtime DATETIME NOT NULL,closedtime DATETIME,milestone_id INTEGER,
  ...
  while(True):
    url = 'https://api.github.com/repos/'+repo+'/milestones?state=all&page=' + str(page)
  ...
  issues = dict()
  while(True):
    url = 'https://api.github.com/repos/'+repo+'/issues?state=all&page=' + str(page)
    doNext = dumpIs(url, issues, token)
  ...
  while(True):
    ...
    print("issue event page "+ str(page))
    page += 1
    if not doNext : break
  page = 1
  comments = []
  ...
  for issue, issueObj in issues.iteritems():
    events = issueObj[1]
    issueTuples.append([issue, issueObj[0],issueObj[2],issueObj[3],issueObj[4],issueObj[5],issueObj[6],issueObj[7]]);
