...
def dump1(u,issues):
  token = "0c3f4a6863127fef539c482af166a2bd1edbde06" # <===
  ...
  for event in w:
    ...
    issues[issue_number] = all_events
  return True
...
def launchDump():
  ...
  for issue, events in issues.iteritems():
    ...
    for event in events: 
      print(event.show())
      # data=(event.issue_id, event.action, event.user, event.commitID, event.milestone, event.state)
      # cur.execute("""INSERT into issues values(%s, %s, %s, %s, %s,%s);
      #             """, data)
      # db.commit()
    print('')
